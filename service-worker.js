/* PocketPal Service Worker v5.30 */
const CACHE = 'pocketpal-v530';
...
function dueAlarms(s, now) {
  ...
  if ((s.poopCount || 0) >= 1) {
    out.push({ type: 'poop', title: 'PocketPal made a mess', body: 'A pile on the screen — please clean.' });
  }
  if (s.sick) {
    out.push({ type: 'sick', title: 'PocketPal feels sick', body: 'I need medicine.' });
  }
  if (s.lightsMiss) {
    out.push({ type: 'lights', title: 'PocketPal is sleepy', body: 'Yawning — lights off for bed.' });
  }
  if (s.naughty) {
...
self.addEventListener('fetch', event => {
  ...
  const isPage = req.mode === 'navigate' || dest === 'document' || url.pathname.endsWith('.html') || url.pathname.endsWith('/');
